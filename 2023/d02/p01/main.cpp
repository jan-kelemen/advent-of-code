...
int main([[maybe_unused]] int argc, char** argv)
{
    ...
    auto const filter = [](game_set const& game)
    {
        return std::all_of(game.sets.cbegin(),
            game.sets.cend(),
            [](cube_set const& c) { return c.red <= 12 && c.green <= 13 && c.blue <= 14; });
    };

    std::uint64_t id_sum{};
    for (auto const& g : games.value() | std::views::filter(filter))
    {
        id_sum += g.id;
    }
    std::cout << id_sum << '\n';
