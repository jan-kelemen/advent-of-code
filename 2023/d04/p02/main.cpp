...
int main([[maybe_unused]] int argc, char** argv)
{
    ...
    std::vector<std::pair<int, std::size_t>> all_cards;
    std::transform(games.value().cbegin(),
        games.value().cend(),
        std::back_inserter(all_cards),
        [](auto const& c)
        {
            ...
            return std::make_pair(c.number, intersect.size());
        });

    std::deque<std::pair<int, std::size_t>> unprocessed{all_cards.cbegin(),
        all_cards.cend()};

    std::size_t total{};
    while (!unprocessed.empty())
    {
        auto card = unprocessed.front();
        unprocessed.pop_front();
        ++total;

        auto it{std::find(all_cards.cbegin(), all_cards.cend(), card)};
        std::copy_n(std::next(it),
            card.second,
