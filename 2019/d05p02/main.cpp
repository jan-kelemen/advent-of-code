...
int64_t const& read(address_mode const mode, int64_t const& position, ram const& ram)
{
  ...
  case address_mode::position:
    return position >= ram.size() ? 0 : ram[position];
...
void write(address_mode const mode, int64_t const& position, int64_t const& value, ram& ram)
{
  ...
  {
  case address_mode::position:
    if (position >= ram.size()) { ram.resize(position + 1); }
    ram[position] = value;
    break;
  case address_mode::immediate:
  default:
...
int main()
{
  ...
  while (true)
  {
    ...
    case instruction_tag::jump_if_true:
    {
      int64_t const& condition = read(first_arg_mode, ram[registers.program_counter] + 1, ram);
      int64_t const& value = read(second_arg_mode, ram[registers.program_counter] + 2, ram);
      if (condition) { registers.program_counter = value; }
      else { registers.program_counter += 3; }
      break;
    }
    case instruction_tag::jump_if_false:
    {
      int64_t const& condition = read(first_arg_mode, ram[registers.program_counter] + 1, ram);
      int64_t const& value = read(second_arg_mode, ram[registers.program_counter] + 2, ram);
      if (!condition) { registers.program_counter = value; }
    ...
    case instruction_tag::less_than:
    {
      int64_t const& lhs = read(first_arg_mode, ram[registers.program_counter + 1], ram);
      int64_t const& rhs = read(second_arg_mode, ram[registers.program_counter + 2], ram);
      write(third_arg_mode, ram[registers.program_counter + 3], static_cast<int64_t>(lhs < rhs), ram);
      registers.program_counter += 4;
      break;
    }
    case instruction_tag::equals:
    {
      int64_t const& lhs = read(first_arg_mode, ram[registers.program_counter + 1], ram);
      int64_t const& rhs = read(second_arg_mode, ram[registers.program_counter + 2], ram);
      write(third_arg_mode, ram[registers.program_counter + 3], static_cast<int64_t>(lhs == rhs), ram);
